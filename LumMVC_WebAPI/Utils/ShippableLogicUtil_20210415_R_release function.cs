...
namespace LumMVC_WebAPI.Utils
{
    public class ShippableLogicUtil
    {
        ...
        {
            List<string> testList = new List<string>() { "P42011500604" };
        ...
        private bool SaveItemResult(string rwId, string waferId, Dictionary<string, string> resDic)
        {
            ...
            try
            {
                //Always check hold comment is exist or not
                var holdInfo = (from h in _DB.TBL_WAFER_RESUME
                                join i in _DB.TBL_WAFER_RESUME_ITEM on h.Id equals i.Header_Id
                                where (h.RW_Wafer_Id == rwId || (h.Wafer_Id == waferId && string.IsNullOrEmpty(h.RW_Wafer_Id))) && h.Type == "Hold"
                                select i).ToList().OrderByDescending(r=> r.Last_Updated_Date);

                if (holdInfo != null && holdInfo.Count() > 0)
                {
                    foreach (var hi in holdInfo)
                    {
                        if (resDic.ContainsKey(hi.Key) == false)
                            resDic.Add(hi.Key, hi.Value);
                    }
                }

                if (resDic.Count() == 0 || (resDic.ContainsKey("IsHold(Y/N)") && resDic.ContainsValue("R")))
                {
                    isPassed = "Y";
                }

                result = _JsonHelper.JsonTextBuilder(resDic);
